...
// #define USE_WS2812_LED_ADAFRUIT                                             // built in WS2812 LED Adafruit variant (requires AdaFruit_NeoPixel libray)
// #define USE_MONO_LED                                                        // built in mono LED (some of ESP32 DEV boards)
...
Neither object creation nor any initiation is required; just put LED(status) in your code.
in a place where you need to update the LED status
To change USER behavior, use macros:
LEDUSER(R,G,B,I)
 where R,G,B are RGB color components, but I is the blinking interval in milliseconds
See examples:
